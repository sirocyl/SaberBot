      ...
      command(:kick, description: "Kick a user. Staff only.\nUsage: `!kick <user> <reason>`", min_args: 1) do |event|
        ...
        if event.message.author.permission?(:kick_members)
          if event.message.mentions[0]
            ...
            details = {sid: event.server.id, mention: member.mention, staff: event.message.author.mention, reason: reason}
            ...
            Server_channels[event.server][Config["modlog_channel"]].send("**Kicked:** #{details[:mention]}\n**Reason:** #{details[:reason]}\n**Responsible Moderator:** #{details[:staff]}")
            nil
          else
